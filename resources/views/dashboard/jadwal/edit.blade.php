...
@section('main_content')
    <div class="p-12 py-6">
        ...
        <h1 class="text-2xl font-bold my-5">Ubah Jadwal</h1>
        <form action="/dashboard/jadwal/{{ $jadwal->id }}" method="POST">
            @csrf
            @method('PUT')
            <div class="flex flex-col gap-3">
                <label for="nama">Nama</label>
                <input value="{{ $jadwal->nama }}" required type="text" name="nama" id="nama"
            ...
            <div class="flex flex-col gap-3 mt-5">
                ...
                <input value="{{ $jadwal->hari }}" required type="text" name="hari" id="hari"
                    class="border border-gray-300 rounded-lg p-2">
                {{-- Show error text --}}
                @error('hari')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
            ...
            <div class="flex flex-col gap-3 mt-5">
                ...
                <input value="{{ $jadwal->jam_mulai }}" required type="time" name="jam_mulai" id="jam_mulai"
                    class="border border-gray-300 rounded-lg p-2">
                {{-- Show error text --}}
                @error('jam_mulai')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
            ...
            <div class="flex flex-col gap-3 mt-5">
                ...
                <input value="{{ $jadwal->jam_selesai }}" required type="time" name="jam_selesai" id="jam_selesai"
                    class="border border-gray-300 rounded-lg p-2">
                {{-- Show error text --}}
                @error('jam_selesai')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
            ...
            <div class="flex flex-col gap-3 mt-5">
                ...
                <select required name="lab_id" id="lab" class="border border-gray-300 rounded-lg p-2">
                    ...
                    @foreach ($labs as $lab)
                        <option value="{{ $lab->id }}" @if ($lab->id == $jadwal->lab_id) selected @endif>
                            {{ $lab->nama }}</option>
                    @endforeach
                </select>
                {{-- Show error text --}}
            ...
            <div class="flex flex-col gap-3 mt-5">
                ...
                <select required name="matkul_id" id="matkul" class="border border-gray-300 rounded-lg p-2">
                    ...
                    @foreach ($matkuls as $matkul)
                        <option value="{{ $matkul->id }}" @if ($matkul->id == $jadwal->matkul_id) selected @endif>
                ...
                <div class="flex flex-col gap-3 mt-5">
                    ...
                    <select required name="asprak_1" id="asprak_1" class="border border-gray-300 rounded-lg p-2">
                        <option value="" disabled>Pilih Asisten Praktikum 1</option>
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" @if ($user->id == $jadwal->asprak_1) selected @endif>
                                {{ $user->nama }}</option>
                        @endforeach
                    </select>
                    {{-- Show error text --}}
                ...
                <div class="flex flex-col gap-3 mt-5">
                    ...
                    <select name="asprak_2" id="asprak_2" class="border border-gray-300 rounded-lg p-2">
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" @if ($user->id == $jadwal->asprak_2) selected @endif>
                                {{ $user->nama }}</option>
                        @endforeach
                    </select>
                    {{-- Show error text --}}
